        ...
        return provider
            ...
            .flatMap { [unowned self] result -> Observable<Result<SquadDetail, GeneralError>> in
                ...
                case .success(let detail):
                    ...
                    let flicksObservable: Observable<[FlickModel]> = self.provider.request(target: .getPageListWithFlick(squadId: self.currentSquadId, pageIndex: 1, pageSize: 10, keyword: ""), model: GeneralModel.List<FlickModel>.self, atKeyPath: .data).asObservable().map {
                        switch $0 {
